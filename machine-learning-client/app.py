...
def load_cat_detector():
    ...
    cat_cascade_ext_path = os.path.join(CASCADE_PATH, "haarcascade_frontalcatface_extended.xml")
...
def detect_cat_with_cascade(image):
    ...
    cats = cat_cascade.detectMultiScale(gray, scaleFactor=1.1, minNeighbors=5, minSize=(40, 40))
    
    # If no cats detected with the first cascade, try the extended one with increased sensitivity
    if len(cats) == 0:
        cats = cat_cascade_ext.detectMultiScale(gray, scaleFactor=1.1, minNeighbors=5, minSize=(40, 40))
    ...
    try:
        ...
        if is_cat_detected:
            ...
            if recent_detection:
                ...
                print(f"Cat detected, but within cooldown period. Skipping event logging.")
                return jsonify({
            ...
            print(f"Cat detected with {confidence:.2f} confidence! Event saved at {event['timestamp'].strftime('%H:%M:%S')}")
